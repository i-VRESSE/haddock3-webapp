...
export function UserProfileInputDTOFromJSONTyped(
  json: any,
  ...
  return {
    email: json["email"],
    oauthAccounts: (json["oauth_accounts"] as Array<any>).map(
      OAuthAccountNameFromJSON
    ),
    roles: json["roles"],
  };
...
export function UserProfileInputDTOToJSON(
  value?: UserProfileInputDTO | null
): any {
  if (value === undefined) {
    return undefined;
  }
  ...
  return {
    email: value.email,
    oauth_accounts: (value.oauthAccounts as Array<any>).map(
      OAuthAccountNameToJSON
    ),
    roles: value.roles,
  };
